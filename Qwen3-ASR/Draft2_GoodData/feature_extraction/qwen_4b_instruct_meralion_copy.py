...
INPUT_DIR = "filtered_meralion_transcriptions"
OUTPUT_DIR = "qwen_instruct_meralion_results"
os.makedirs(OUTPUT_DIR, exist_ok=True)


neutral_files = [f for f in os.listdir(INPUT_DIR) if f.endswith("ml_30_dataset_neutral.json")]
...
for filename in neutral_files:
    ...
    output_filename = filename.replace("_neutral.json", "_instruct_analysis.json")
    ...
    for i in tqdm(range(0, len(data_to_process), batch_size), desc=f"Analyzing {filename}"):
        ...
        try:
            ...
            # This ensures the GPU has actually finished clearing before the next batch starts
